...
te_transforms_imageC = transforms.Compose([transforms.CenterCrop(224),
									transforms.ToTensor(),
									normalize])
te_transforms_imageC_transform = transforms.Compose([transforms.CenterCrop(224),
									transforms.ToTensor(),
									normalize])
...
class SelectedRotateImageFolder(datasets.ImageFolder):
    def __init__(self, root, train_transform, second_transform=None, original=True, rotation=True, rotation_transform=None):
    ...
    def __getitem__(self, index):
        ...
        if self.second_transform is not None:
            img_aug = self.second_transform(img_input)
        else:
            img_aug = img_input  
...
def prepare_test_data(args, use_transforms=True):	
    if args.corruption == 'original'or args.corruption == 'ia'or args.corruption == 'ir'or args.corruption == 'is':
        te_transforms_local = te_transforms if use_transforms else None
        te_transforms_local_transform = te_transforms_imageC_transform if use_transforms else None
    ...
    if not hasattr(args, 'corruption') or args.corruption == 'original':
        ...
        teset = SelectedRotateImageFolder(validdir, train_transform=te_transforms_local, second_transform=te_transforms_local_transform,
                                           original=False, rotation=False,rotation_transform=rotation_te_transforms)
    elif args.corruption in common_corruptions:
        ...
        if args.twomodel:
            ...
            if args.bothcnn:
                teset = SelectedRotateImageFolder(validdir, train_transform=te_transforms_local,second_transform=te_transforms_local,
                original=False, rotation=False, rotation_transform=rotation_te_transforms)
            else:
                teset = SelectedRotateImageFolder(validdir, train_transform=te_transforms_local_transform,second_transform=te_transforms_local_transform,
                original=False, rotation=False, rotation_transform=rotation_te_transforms)
    elif args.corruption == 'ir':
        validdir = args.data_ir
        teset = datasets.ImageFolder(validdir, te_transforms_local)
    elif args.corruption == 'is':
    ...
    elif args.corruption == 'ia':
        validdir = args.data_ia
        teset = datasets.ImageFolder(validdir, te_transforms_local)    
    else:
