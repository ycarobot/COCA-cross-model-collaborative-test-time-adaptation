...
class EATA(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.steps > 0:
            for _ in range(self.steps):
                outputs, num_counts_2, num_counts_1, updated_probs = forward_and_adapt_eata(x, self.net, self.optimizer, self.fishers, self.e_margin, self.current_model_probs,self.imagenet_mask, fisher_alpha=self.fisher_alpha, num_samples_update=self.num_samples_update_2, d_margin=self.d_margin)
    ...
    def reset(self):
        ...
        load_model_and_optimizer(self.net, self.optimizer,
                                 self.net_state, self.optimizer_state)
